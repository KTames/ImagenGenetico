...
class HtmlOutput:
    ...
    def addInGeneration(self, sector):
        output = ""
        sectorSize = sector.maxX - sector.minX
        colorDistribution = sector.getColors()
        size = sector.squareSize
        
        for key, color in colorDistribution.items():
            r, g, b = color.rgbColor
            for _ in range(0, color.squareCount):
                output += "<rect "
    ...
    def write(self):
        ...
        for generation in range(0, len(self.output[1])):
            file = open("output/output-" + str(generation) + ".svg", "w")
            file.truncate()
            file.write('<svg baseProfile="tiny" height="100%" version="1.2" width="100%" xmlns="http://www.w3.org/2000/svg" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 1024 1024"><defs />' + self.output[1][generation] + "</svg>")
            file.close()
